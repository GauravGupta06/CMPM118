...
class SNNModel:
    ...
    def train_model(self, train_loader, test_loader, num_epochs=150, print_every=15):
        ...
        Args:
            ...
            spike_lam: Lambda for spike regularization
    ...
    def save_model(self, base_path="results", counter_file="experiment_counter.txt"):
        ...
        model_save_path = f"{base_path}/{subdir}/models/{model_name}_Take{num}_{self.w}x{self.h}_T{self.n_frames}_B{self.beta}_SpkLam{self.spike_lam}_Epochs{self.epochs}.pth"
        graph_save_path = f"{base_path}/{subdir}/graphs/{model_name}_Take{num}_{self.w}x{self.h}_T{self.n_frames}_B{self.beta}_SpkLam{self.spike_lam}_Epochs{self.epochs}.png"
