...
def main():
    ...
    # Sparse Model Defaults
    parser.add_argument('--sparse_tau_mem', type=float, default=0.005)
    ...
    train_loader = DataLoader(cached_train, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=args.num_workers, pin_memory=(device.type == "cuda"))
    test_loader = DataLoader(cached_test, batch_size=args.batch_size, shuffle=False, drop_last=True, num_workers=args.num_workers, pin_memory=(device.type == "cuda"))

    # Hyperparameters (sparse vs dense) defaults
    if args.model_type == 'sparse':
        tau_mem = args.sparse_tau_mem
