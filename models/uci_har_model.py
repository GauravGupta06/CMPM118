...
class UCIHARSNN:
    ...
    def __init__(self, input_size=9, n_frames=128, tau_mem=0.1, tau_syn=0.05, spike_lam=0.0,
                 model_type="dense", device=None, num_classes=6, lr=0.001, dt=0.02, threshold=1.0, has_bias=True):
        ...
        # Build network: 9 → 128 → 64 → 32 → 6
        self.net = Sequential(
            LinearTorch((input_size, 128), has_bias=has_bias),
            LIFTorch(128, tau_mem=Constant(tau_mem), tau_syn=Constant(tau_syn), 
                     threshold=Constant(threshold), bias=Constant(0.0), dt=dt, has_rec=False),
            LinearTorch((128, 64), has_bias=has_bias),
            LIFTorch(64, tau_mem=Constant(tau_mem), tau_syn=Constant(tau_syn), 
                     threshold=Constant(threshold), bias=Constant(0.0), dt=dt, has_rec=True),
            ...
            LIFTorch(num_classes, tau_mem=Constant(tau_mem), tau_syn=Constant(tau_syn), 
                     threshold=Constant(threshold), bias=Constant(0.0), dt=dt, has_rec=False),
        ).to(self.device)
