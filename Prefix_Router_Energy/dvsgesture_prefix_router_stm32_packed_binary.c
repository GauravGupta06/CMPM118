/* ============================================================
 * STM32F411CEU6 (Black Pill) - DVSGesture Packed-Binary Prefix Router
 *
 * Measures a binary-clipped router metric from the first 400 ms of the
 * DVSGesture model input representation:
 *
 *     [400, 2, 32, 32] count frames clipped to binary occupancy
 *     400 * 2 * 32 * 32 = 819200 bits = 102400 packed bytes
 *
 * The trained software SNN currently uses count frames, not binary frames.
 * This firmware measures the cost of a binary occupancy metric:
 *
 *     count_frame[t, polarity, y, x] > 0
 *
 * The host sends one bit-packed sample as hex chunks:
 *
 *     BEGIN 102400 819200
 *     DATA <hex bytes>
 *     DATA <hex bytes>
 ...
 * region only popcounts the packed binary prefix and applies a threshold:
 *
 *     binary_prefix_ones >= 10000 -> dense
 *
 * The threshold is a placeholder; select the final threshold after router
 * AUC/accuracy-energy analysis. It has negligible effect on the measured
 * cycle count compared with the popcount loop.
 *
 * RAM note: 102400 bytes is large for STM32F411CEU6. If the linker reports
 * SRAM overflow, reduce PREFIX_MS on the host/firmware or switch to a
 * streaming/event-count firmware.
 * ============================================================ */
...
static int8_t CDC_Receive_FS(uint8_t *Buf, uint32_t *Len)
{
    for (uint32_t i = 0; i < *Len; i++) {
        ...
        } else if (!cdc_rx_ready && cdc_rx_len < (MAX_LINE_LEN - 1)) {
